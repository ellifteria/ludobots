...
import pyrosim_modded.pyrosim_modded as pyrosim_modded

import pyrosim_modded.constants as c
...
class NEURON: 
    ...
    def Update_Sensor_Neuron(self):
        self.Set_Value(pyrosim_modded.Get_Touch_Sensor_Value_For_Link(self.Get_Link_Name()))
    ...
    def Update_Hidden_Or_Motor_Neuron(self, neurons, synapses):
        ...
        for synapse in synapses:
            if synapse[1] == str(self.Get_Name()):
                self.Allow_Presynaptic_Neuron_To_Influence_Me(synapses[synapse[1]].Get_Weight(), neurons[synapse[0]].Get_Value())
        exit()

    def Allow_Presynaptic_Neuron_To_Influence_Me(self, weight, pre_synaptic_neuron_value):
        print(weight)
        print(pre_synaptic_neuron_value)
        exit()
